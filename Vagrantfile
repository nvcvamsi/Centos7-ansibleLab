...
Vagrant.configure("2") do |config|
  ...
  (1..NUM_WORKER_NODE).each do |i|
	...
	node.vm.provision "user-add", type: "shell", :path => "centos/userAdd.sh"
    end
  end
  ...
  (1..NUM_MASTER_NODE).each do |i|
      config.vm.define "Ansiblemaster" do |node|
        ...
        node.vm.provision "setup-dns", type: "shell", :path => "centos/update-dns.sh"
	node.vm.provision "install_package", type: "shell", :path => "centos/vagrant/packges_install.sh"
	node.vm.provision "add-users", type: "shell", :path => "centos/userAdd.sh"
	node.vm.provision "ssh-keyexch", type: "shell", :path => "centos/vagrant/ssh-copy-node.sh"
